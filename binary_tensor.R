...
#########################
binary_fit=function(Y,r,alpha=10,sigma=1,option=2){
    Y0=Y
    ...
    if(r<=min(dim(Y))){ ## if rank is less than the minimum dimension
        ...
        M=tensorize(A,B,C)
        if(max(abs(M))>alpha/sigma){
            C=C/max(abs(M))*(alpha/sigma)
        }
        start=list("A"=A,"B"=B,"C"=C)
        ## binary tensor decomposition
        fit0=binary_fit_fix(Y0,r,alpha/sigma,start,option)
        cost0=fit0$cost
        ...
        for(n in 1:nrand){
            ...
            ## binary tensor decomposition
            fit=binary_fit_fix(Y0,r,alpha/sigma,start,option)
            cost=fit$cost
        ...
        for(n in 1:nrand){
            ...
            start=list("A"=start$A,"B"=start$B,"C"=start$C)
            ## binary tensor decomposition
            fit=binary_fit_fix(Y0,r,alpha/sigma,start)
            cost=fit$cost
            if(max(cost)>max(cost0)){
                fit0=fit
                cost0=cost
...
binary_fit_fix=function(Y,r,alpha,start,option=2){
    missing=which(is.na(Y)==T)
    nonmissing=which(is.na(Y)==F)
    Y[missing]=mean(Y,na.rm=T)
    ...
    d=dim(Y)
    nloop=0
    while((abs((cost_new-cost)/cost)>=0.001)&(nloop<50)){
        nloop=nloop+1
        A0=A
        B0=B
        C0=C
        ...
        if(max(abs(M))>alpha){ ## project into the feasible space
            ...
            if(min(upper,lower)>0){
                temp=optimize(function(x)search_new(x,A,B,C,Z,A0,B0,C0,nonmissing),lower=0,upper=min(upper,lower),maximum=TRUE)
                gamma=temp$maximum
            }
            else gamma=0
